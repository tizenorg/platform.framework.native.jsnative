...
void AlarmInstance::HandleMessage(const char* msg) {
  ...
  if (cmd == "getAlarms") {
    HandleGetAlarms(request["asyncid"].to_str());
  }
}
...
void AlarmInstance::HandleSyncMessage(const char* msg) {
  ...
  if (cmd == "scheduleAfter") {
    ...
    HandleAfterSchduleAfter(target, appcontrol_json, after);
    ...
    HandleAtDate(target, appcontrol_json, date, weeks);
  } else if (cmd == "remove") {
...
void AlarmInstance::HandleAfterSchduleAfter(const std::string& target,
                                            const std::string& appcontrol_json,
                                            int after) {
  ...
  app_control_set_app_id(appcontrol, target.c_str());
...
void AlarmInstance::HandleAtDate(const std::string& target,
                                 const std::string& appcontrol_json,
                                 int64_t date,
                                 int weeks) {
  picojson::object result;
...
void AlarmInstance::HandleGetAlarms(const std::string& asyncid) {
  std::thread task([asyncid, this]() {
    picojson::object result;
    picojson::array arr;
    result["asyncid"] = picojson::value(asyncid);

    int ret = alarm_foreach_registered_alarm([](int alarm_id, void* user_data) {
      picojson::array& arr = *(static_cast<picojson::array*>(user_data));
      arr.push_back(picojson::value(std::to_string(alarm_id)));
      return true;
    }, &arr);
    ...
    char* json = strdup(picojson::value(result).serialize().c_str());
    CallbackData* user_data = new CallbackData();
    user_data->json = json;
    user_data->self = this;
    g_idle_add([](void* user_data) {
      CallbackData* data = static_cast<CallbackData*>(user_data);
      data->self->PostMessage(data->json);
      free(data->json);
      delete data;
      return 0;
    }, user_data);
