 ...
 * app_manager.getInstalledApplications().then(function(){ ... });
 *
...
class AppManager extends EE {
   ...
   *
   * @param {Object} [filter]
   * You can use property of InstalledApplication to filter out
   *
   ...
   * ```
   * var app_manager = require('tizen-app-manager');
   * app_manager.getInstalledApplications({'type':'jsapp'})
   *   .then(function(apps) {
   ...
   *    });
   *
   * app_manager.getInstalledApplications({'type':'jsapp', 'disabled':true})
   *   .then(function(apps) {
   *      apps.forEach(function(app) {
   ...
   * ```
   */
  getInstalledApplications(filter) {
  ...
  /**
   * @method getInstalledApplication
   *
   * @param {string} appid
   *
   ...
   *  If does not existed, undefined will returned
   */
  getRunningApplication(appidOrPid) {
    var args = {};
  ...
  /**
   * @method isRunningApplication
   *
   * @param {string | Number} appid or pid
   * @return {boolean}
   */
  isRunningApplication(appidOrPid) {
    return getRunningApplication(appidOrPid) != undefined;
  }
