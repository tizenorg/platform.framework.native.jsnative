...
void DeviceInstance::HandleLEDPlay(
  ...
  bool dutyOn = args.get("dutyOn", false).asBool();

  int flags = LED_CUSTOM_DEFAULT;
  if (dutyOn) {
    flags |= LED_CUSTOM_DUTY_ON;
  }

  int ret;
  if ((ret = device_led_play_custom(timeOn, timeOff, color, flags)) < 0) {
    const char* errmsg = GetErrorMessage(ret);
