...
/**
 * The Device module provides functions to control devices or to get status
 * of devices. An instance of the {{#crossLink "Device"}}{{/crossLink}} class
 * is returned when require 'tizen-device'.
 * ```
...
/**
 * The Device class has sub modules to control each device or to get status of
 * each device.
 *
 * An instance of the this class is returned when require
 * 'tizen-device'.
 * ```
 * var device = require('tizen-device');
 * ```
 ...
 * {{#crossLink "Device/battery:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var battery = require('tizen-device').battery;
 * ```
 * @class Battery
 * @since 3.0
 * @namespace Device
...
class Battery extends EE {
   ...
   * The value will be true if the battery is charging, else false
   *
   ...
   * ```
   * var device = require('tizen-device');
   * device.battery.on('change', function(isCharging, level, capacity) {
   *   console.log('battery status has changed');
   ...
   * @value 'high' The battery is charged
   * @value 'full' The battery status is full
   * @param {Number} capacity The changed battery percentage
   */
  __event_handle__(ev) {
    var changed = false;
 ...
 *
 * An instance of this class can be accessed with the
 * {{#crossLink "Device/display:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var display = require('tizen-device').display;
 * ```
 * @class Display
 * @since 3.0
 * @namespace Device
...
class Display extends EE {
  ...
  /**
   * Gets or sets the current display state.
   *
   * @attribute state
   * @type {String}
   * @value 'normal' Normal state
   * @value 'dim' Screen dim state
   * @value 'off' Screen off state
   * @privilege http://tizen.org/privilege/display
   */
  get state() {
  ...
  set state(value) {
    ...
    if (['normal', 'dim', 'off'].indexOf(value_v) >= 0) {
   ...
   *        It can be greater than or equal to 0 and less than the
   *        {{#crossLink "Device.Display/count:attribute"}}{{/crossLink}}.
   ...
   * ```
   * var device = require('tizen-device');
   * device.display.on('change', function(state) {
  ...
  __event_handle__(ev) {
    if (ev['event'] === 'dispaly.state') {
      ...
      this.emit('change', this[DISPLAY_STATE]);
    }
  }
};
 ...
 *
 * An instance of this class can be accessed with the
 * {{#crossLink "Device/haptic:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var haptic = require('tizen-device').haptic;
 * ```
 * @class Haptic
 * @since 3.0
 * @namespace Device
 ...
 *
 * An instance of this class can be accessed with the
 * {{#crossLink "Device/flash:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var flash = require('tizen-device').flash;
 * ```
 * @class Flash
 * @since 3.0
 * @namespace Device
...
class Flash extends EE {
  ...
  /**
   * Gets or sets the brightness value of a flash device.
   *
   ...
   * ```
   * @event changed
   * @param {Number} brightness The changed brightness value
   */
  __event_handle__(ev) {
 ...
 *
 * An instance of this class can be accessed with the
 * {{#crossLink "Device/led:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var led = require('tizen-device').led;
 * ```
 * @class Led
 * @since 3.0
 * @namespace Device
...
class Led {
  /**
   * Plays the custom effect of the LED device.
   *
   * @method play
   * @param {Number} timeOn Turn on time in milliseconds
   * @param {Number} timeOff Turn off time in milliseconds
   * @param {String} color The Color value, RGB in hex, eg '#00FF00'
   * @privilege http://tizen.org/privilege/led
   */
  play(timeOn, timeOff, color) {
  ...
  /**
   * Stops the custom effect of the LED device.
   *
   * @method stop
   * @privilege http://tizen.org/privilege/led
   */
 ...
 *
 * An instance of this class can be accessed with the
 * {{#crossLink "Device/power:attribute"}}{{/crossLink}} attribute of the
 * {{#crossLink "Device"}}{{/crossLink}} class.
 * ```
 * var power = require('tizen-device').power;
 * ```
 * @class Power
 * @since 3.0
 * @namespace Device
