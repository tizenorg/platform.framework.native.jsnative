...
class MessagePort {
   ...
   * });
   * ```
   * Each port is different from each own 'trusted' value.
   * For example,
   * ``` javascript
   * messageport.listen('port1', cb){});  // A port which has its name 'port1' and is not
   *                                      // trusted port(because trusted's default value
   *                                      // is false if it's skipped).
   * messageport.listen('port1', true, cb){});  // Other port which has its name 'port1'
   *                                            // and is trusted port.
   * ```
   * @method listen
   * @param {String} portName Port name to listen message from other port.
   * @param {Boolean} [trusted=false] Is this port to listen trusted
   * port? This is skipped and its default value is false.
   * @param {Function} callback Callback function to be
   * invoked when this port receives any message.
   * @since 3.0
   */
   ...
   * For example,
   * ``` javascript
   * messageport.stopListen('port1');  // A port which has its name 'port1' and is not
   *                                   // trusted port(because trusted's default value
   *                                   // is false if it's skipped).
   * messageport.stopListen('port1', true);  // Other port which has its name 'port1'
   *                                         // and is trusted port.
   * ```
   * @method stopListen
   * @param {String} portName Port name to stop listening message from other
   * port.
   * @param [trusted=false] {Boolean} Is this port to listen trusted
   * port? This is skipped and false is default.
   * @since 3.0
   */
  ...
  /**
   * Send a message or messages to a port
   * ```javascript
   ...
   * @param {String} receiver.appId Application Id to be sent message.
   * @param {String} receiver.portName Name of port which is sent message.
   * This port should be in the application which has the appId.
   * @param {Boolean} [receiver.trusted=false] Value whether
   * the port is trusted port or not. False is default.
   * @param {Map|Object} message Message to send to receiver. Its type can be
   * both Map and Object.
   * * Be carefull about message's contents.
   * * Currently, only string value and string array value are supported.
   * @param {Object} [sender] Sender to send this message.
   ...
   * that in this JSNative app, you use listen function with this port name.
   * If portName is a port name which is not used by listen(), send() sends
   * message without sender information.
   * @parma {Boolean} [sender.trusted=false] Value whether the
   * port is trusted port or not. False is default.
   * If you want to inform receiver to know who this message sends. You can skip
   * this parameter.
