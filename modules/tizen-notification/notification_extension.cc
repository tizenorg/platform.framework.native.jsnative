...
void NotificationInstance::PostOrUpdate(picojson::value& value, bool isUpdate) {
    ...
    if (led.compare("LED_OFF") == 0) {
      ...
      if (dictionary.contains("customColor")) {
        ret = notification_set_led(noti, NOTIFICATION_LED_OP_ON_CUSTOM_COLOR,
                                   (int)dictionary.get("customColor")
                                                  .get<double>());
        if (ret != NOTIFICATION_ERROR_NONE) {
          LOGD("@ led set FAIL");
        }
        LOGD("@ led custom color set %s ", (int)dictionary.get("customColor")
                                                          .get<double>());
      } else {
      ...
      if (dictionary.contains("ledOnPeriod")
        ...
        LOGD("@ ledOnPeriod set %d ", (int)dictionary.get("ledOnPeriod")
                                                     .get<double>());
        LOGD("@ ledOffPeriod set %d ", (int)dictionary.get("ledOffPeriod")
                                                      .get<double>());
      }
    }
  }
  ...
  if (dictionary.contains("layout")) {
    ...
    } else if (dictionary.get("layout").to_str() == "EVENT_SINGLE") {
      layout = NOTIFICATION_LY_NOTI_EVENT_SINGLE;
    } else if (dictionary.get("layout").to_str() == "EVENT_MULTIPLE") {
    ...
    } else if (dictionary.get("layout").to_str() == "ONGOING_EVENT") {
      layout = NOTIFICATION_LY_ONGOING_EVENT;
    } else if (dictionary.get("layout").to_str() == "ONGOING_PROGRESS") {
...
void NotificationInstance::GetNotification(const std::string& async_id,
  ...
  if (layout == NOTIFICATION_LY_NONE) {
    ...
    obj["layout"] = picojson::value("EVENT_SINGLE");
  } else if (layout == NOTIFICATION_LY_NOTI_EVENT_MULTIPLE) {
    obj["layout"] = picojson::value("EVENT_MULTIPLE");
    ...
    obj["layout"] = picojson::value("ONGOING_EVENT");
  } else if (layout == NOTIFICATION_LY_ONGOING_PROGRESS) {
    obj["layout"] = picojson::value("ONGOING_PROGRESS");
  }
