...
void NotificationInstance::AddButton(const notification_h& noti,
                                     const picojson::value& appCtrl,
                                     const notification_button_index_e
                                       buttonIndex,
                                     const notification_text_type_e
                                       buttonTextType,
...
void NotificationInstance::PostOrUpdate(const picojson::value& value,
                                        bool isUpdate) {
  ...
  if (isUpdate || (id > 0)) {
    ...
    if (type == "type-noti") {
      LOGD("Create notification, type: type-noti");
      ...
      LOGD("Create notification, type: type-active");
      noti = notification_create(NOTIFICATION_TYPE_NOTI);
  ...
  if (dictionary.contains("iconForIndicatorPath")) {
    ret = notification_set_image(noti,
                                 ...
                                 dictionary.get("iconForIndicatorPath")
                                           .to_str().c_str());
  ...
  if (dictionary.contains("iconForLockPath")) {
    ret = notification_set_image(noti, NOTIFICATION_IMAGE_TYPE_ICON_FOR_LOCK,
                                 dictionary.get("iconForLockPath").to_str()
                                 .c_str());
    LOGD("@ iconForLockPath set : %s", dictionary.get("iconForLockPath")
                                       .to_str().c_str());
    if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (dictionary.contains("thumbnailPath")) {
    ret = notification_set_image(noti, NOTIFICATION_IMAGE_TYPE_THUMBNAIL,
                                 dictionary.get("thumbnailPath").to_str()
                                 .c_str());
    LOGD("@ thumbnailPath set :  %s", dictionary.get("thumbnailPath")
                                      .to_str().c_str());
    if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (dictionary.contains("subIconPath")) {
    ret = notification_set_image(noti, NOTIFICATION_IMAGE_TYPE_ICON_SUB ,
                                 dictionary.get("subIconPath").to_str()
                                 .c_str());
    LOGD("@ subIconPath set :  %s", dictionary.get("subIconPath").to_str()
                                    .c_str());
    if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (dictionary.contains("backgroundImagePath")) {
    ret = notification_set_image(noti, NOTIFICATION_IMAGE_TYPE_BACKGROUND,
                                 dictionary.get("backgroundImagePath")
                                 .to_str().c_str());
    LOGD("@ backgroundImagePath set:  %s", dictionary.get("backgroundImagePath")
                                            .to_str().c_str());
    if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (dictionary.contains("content")) {
    ret = notification_set_text(noti, NOTIFICATION_TEXT_TYPE_CONTENT ,
                                dictionary.get("content").to_str().c_str(),
  ...
  if (dictionary.contains("timestamp")) {
    ...
    ret = notification_set_time(noti, (time_t)dictionary.get("timestamp")
                                                        .get<double>());
    if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (dictionary.contains("vibration")) {
    if (dictionary.get("vibration").to_str().compare("none") == 0) {
      ret = notification_set_vibration(noti, NOTIFICATION_VIBRATION_TYPE_NONE,
                                       NULL);
    } else if (dictionary.get("vibration").to_str().compare("default") == 0) {
      ret = notification_set_vibration(noti,
                                       NOTIFICATION_VIBRATION_TYPE_DEFAULT,
                                       NULL);
    } else {
      ret = notification_set_vibration(noti,
                                       NOTIFICATION_VIBRATION_TYPE_USER_DATA,
                                       dictionary.get("vibration").to_str()
                                       .c_str());
    }
  ...
  if (dictionary.contains("ledOption")) {
    ...
    if (led.compare("off") == 0) {
      ret = notification_set_led(noti, NOTIFICATION_LED_OP_OFF, NULL);
      if (ret != NOTIFICATION_ERROR_NONE) {
        LOGD("@ led set FAIL");
      }
    } else if (led.compare("on") == 0) {
      if (dictionary.contains("customColor")) {
        ret = notification_set_led(noti, NOTIFICATION_LED_OP_ON_CUSTOM_COLOR,
                                   static_cast<int>(dictionary.get("customColor")
                                                             .get<double>()));
        if (ret != NOTIFICATION_ERROR_NONE) {
          LOGD("@ led set FAIL");
        }
        LOGD("@ led custom color set %s ", static_cast<int>(dictionary
                                             .get("customColor").get<double>()));
      } else {
      ...
      if (dictionary.contains("ledOnPeriod")
            && dictionary.contains("ledOffPeriod")) {
        int ledOnPeriod = static_cast<int>(dictionary.get("ledOnPeriod")
                                                    .get<double>());
        int ledOffPeriod = static_cast<int>(dictionary.get("ledOffPeriod")
                                                     .get<double>());
        ret = notification_set_led_time_period(noti, ledOnPeriod, ledOffPeriod);
        if (ret != NOTIFICATION_ERROR_NONE) {
          LOGD("@ led period set FAIL");
        }
        LOGD("@ ledOnPeriod set %d ", static_cast<int>(dictionary
                                        .get("ledOnPeriod").get<double>()));
        LOGD("@ ledOffPeriod set %d ", static_cast<int>(dictionary
                                        .get("ledOffPeriod").get<double>()));
      }
    }
  }
  ...
  if (dictionary.contains("layout")) {
    notification_ly_type_e layout;
    if (dictionary.get("layout").to_str() == "none") {
      layout = NOTIFICATION_LY_NONE;
    } else if (dictionary.get("layout").to_str() == "event-single") {
      layout = NOTIFICATION_LY_NOTI_EVENT_SINGLE;
    } else if (dictionary.get("layout").to_str() == "event-multiple") {
      layout = NOTIFICATION_LY_NOTI_EVENT_MULTIPLE;
    } else if (dictionary.get("layout").to_str() == "thumbnail") {
      layout = NOTIFICATION_LY_NOTI_THUMBNAIL;
    } else if (dictionary.get("layout").to_str() == "ongoing-event") {
      layout = NOTIFICATION_LY_ONGOING_EVENT;
    } else if (dictionary.get("layout").to_str() == "ongoing-progress") {
      layout = NOTIFICATION_LY_ONGOING_PROGRESS;
    }
    ret = notification_set_layout(noti, layout);
    LOGD("@ layout set %d ", layout);
    if (ret != NOTIFICATION_ERROR_NONE) {
      LOGD("@ layout set FAIL");
    }
  }

  if (dictionary.contains("ActionAppControl")) {
    picojson::value appCtrl = dictionary.get("ActionAppControl");
    if (appCtrl.to_str().compare("none") == 0) {
      LOGD("@ no action is set");
    } else {
      const char* appCtrlString = appCtrl.to_str().c_str();
      picojson::value v;
      std::string err;
      ...
      app_control_h appControl;
      app_control_create(&appControl);
      const char* operation = appCtrlObj["__APP_SVC_OP_TYPE__"].to_str()
                              .c_str();
      LOGD("@ appControl operation set %s ", operation);
      app_control_set_operation(appControl, operation);
      auto found = appCtrlObj.find("__APP_SVC_URI__");
      ...
      found = appCtrlObj.find("__APP_SVC_CATEGORY__");
      if (found != appCtrlObj.end()) {
        const char* category = appCtrlObj["__APP_SVC_CATEGORY__"].to_str()
                               .c_str();
        LOGD("@ appControl category set %s ", category);
        app_control_set_category(appControl, category);
      }
      if (dictionary.contains("ActionAppId")) {
        app_control_set_app_id(appControl, dictionary.get("ActionAppId")
                                           .to_str().c_str());
      }

      ret = notification_set_launch_option(noti,
                                         NOTIFICATION_LAUNCH_OPTION_APP_CONTROL,
                                         static_cast<void*>(appControl));
      if (ret != NOTIFICATION_ERROR_NONE) {
        LOGD("@ appControl set FAIL");
      }
    }
  } else {
    LOGD("No ActionAppControl, set default action");
    app_control_h appControl;
    app_control_create(&appControl);
    app_control_set_operation(appControl, APP_CONTROL_OPERATION_DEFAULT);
    char* pkgname = NULL;
    notification_get_pkgname(noti, &pkgname);
    app_control_set_app_id(appControl, pkgname);
    ret = notification_set_launch_option(noti,
                                         NOTIFICATION_LAUNCH_OPTION_APP_CONTROL,
                                         static_cast<void*>(appControl));
    if (ret != NOTIFICATION_ERROR_NONE) {
      LOGD("@ default action set FAIL");
    }
  }
  ...
  if (type == "type-ongoing") {
    if (dictionary.contains("progressSize")) {
      ret = notification_set_size(noti, dictionary.get("progressSize")
                                                  .get<double>());
      LOGD("@ progressSize set %f ", dictionary.get("progressSize")
                                               .get<double>());
      if (ret != NOTIFICATION_ERROR_NONE) {
    ...
    if (dictionary.contains("progressPercentage")) {
      ret = notification_set_progress(noti, dictionary.get("progressPercentage")
                                                      .get<double>()/100.0);
      LOGD("@ progressPercentage set %f ", dictionary.get("progressPercentage")
                                                     .get<double>()/100.0);
      if (ret != NOTIFICATION_ERROR_NONE) {
  ...
  if (type == "type-active") {
    ...
    if (dictionary.get("button1").to_str() == "ADD") {
      ...
      AddButton(noti, appCtrl, buttonIndex, buttonTextType, buttonLabel,
                imagePath);
    }
  ...
  obj["insertionTime"] = picojson::value(static_cast<double>(insertionTime)
                                           * 1000.0);
  SendSyncReply(picojson::value(obj).serialize().c_str());
}

void NotificationInstance::Remove(const int notificationId) {
  picojson::value::object obj;
  int ret = notification_delete_by_priv_id(NULL, NOTIFICATION_TYPE_NONE,
                                           notificationId);
  if (ret != NOTIFICATION_ERROR_NONE) {
    obj["result"] = picojson::value("FAIL");
...
void NotificationInstance::RemoveAll() {
  ...
  if (ret != NOTIFICATION_ERROR_NONE) {
    obj["result"] = picojson::value("FAIL");
    obj["reason"] = picojson::value("RemoveAll(): TYPE_NOTI type failed");
    SendSyncReply(picojson::value(obj).serialize().c_str());
    return;
  }
  ...
  if (ret != NOTIFICATION_ERROR_NONE) {
    obj["result"] = picojson::value("FAIL");
    obj["reason"] = picojson::value("RemoveAll(): TYPE_ONGOING type failed");
    SendSyncReply(picojson::value(obj).serialize().c_str());
    return;
  }
  obj["result"] = picojson::value("OK");
...
void NotificationInstance::GetNotification(const std::string& async_id,
  ...
  if (applist == NOTIFICATION_DISPLAY_APP_ACTIVE) {
    obj["type"] = picojson::value("type-active");
  } else if (notificationType == NOTIFICATION_TYPE_ONGOING) {
    obj["type"] = picojson::value("type-ongoing");
  } else if (notificationType == NOTIFICATION_TYPE_NOTI) {
    obj["type"] = picojson::value("type-noti");
  }
  ...
  notification_get_image(noti_handle, NOTIFICATION_IMAGE_TYPE_ICON_FOR_INDICATOR
                         , &imagePath);
  if (imagePath != NULL) {
    obj["iconForIndicatorPath"] = picojson::value(imagePath);
  }
  notification_get_image(noti_handle, NOTIFICATION_IMAGE_TYPE_ICON_FOR_LOCK,
                         &imagePath);
  if (imagePath != NULL) {
    obj["iconForLockPath"] = picojson::value(imagePath);
  }
  notification_get_image(noti_handle, NOTIFICATION_IMAGE_TYPE_THUMBNAIL,
                         &imagePath);
  if (imagePath != NULL) {
    obj["thumbnailPath"] = picojson::value(imagePath);
  }
  notification_get_image(noti_handle, NOTIFICATION_IMAGE_TYPE_ICON_SUB,
                         &imagePath);
  if (imagePath != NULL) {
    obj["subIconPath"] = picojson::value(imagePath);
  }
  notification_get_image(noti_handle, NOTIFICATION_IMAGE_TYPE_BACKGROUND,
                         &imagePath);
  if (imagePath != NULL) {
  ...
  if (operation == NOTIFICATION_LED_OP_OFF) {
    obj["ledOption"] = picojson::value("off");
  } else {
    obj["ledOption"] = picojson::value("on");
  ...
  notification_get_layout(noti_handle, &layout);
  if (layout == NOTIFICATION_LY_NONE) {
    obj["layout"] = picojson::value("none");
  } else if (layout == NOTIFICATION_LY_NOTI_EVENT_SINGLE) {
    obj["layout"] = picojson::value("event-single");
  } else if (layout == NOTIFICATION_LY_NOTI_EVENT_MULTIPLE) {
    obj["layout"] = picojson::value("event-multiple");
  } else if (layout == NOTIFICATION_LY_NOTI_THUMBNAIL) {
    obj["layout"] = picojson::value("thumbnail");
  } else if (layout == NOTIFICATION_LY_ONGOING_EVENT) {
    obj["layout"] = picojson::value("ongoing-event");
  } else if (layout == NOTIFICATION_LY_ONGOING_PROGRESS) {
    obj["layout"] = picojson::value("ongoing-progress");
  }
  ...
  if (app_control != NULL) {
    ...
    obj["operation_"] = picojson::value(operation);
    char* uri;
    app_control_get_uri(app_control, &uri);
    if (uri != NULL) {
      obj["uri_"] = picojson::value(uri);
    }
    char* mime;
    app_control_get_mime(app_control, &mime);
    if (mime != NULL) {
      obj["mime_"] = picojson::value(mime);
    }
    char* category;
    app_control_get_category(app_control, &category);
    if (category != NULL) {
      obj["category_"] = picojson::value(category);
    }
    char* app_id;
    app_control_get_app_id(app_control, &app_id);
    if (app_id != NULL) {
      obj["ActionAppId"] = picojson::value(app_id);
    }
  }
  obj["result"] = picojson::value("OK");
