...
private:
  // sync
  void PostOrUpdate(const picojson::value& value, bool isUpdate);
  ...
  void AddButton(const notification_h& noti, const picojson::value& appCtrl,
                 const notification_button_index_e buttonIndex,
                 const notification_text_type_e buttonTextType,
                 const std::string label,const std::string imagePath);
