...
class Manager {
  ...
  removeAll() {
    ...
    var result = native_sync_call('removeAll', notification);
    if(result['result'] == 'FAIL') {
      throw Error(result['reason']);
    }
    if(result['result'] == 'OK') {
...
var dictionary = {'title': 'This is the title', 'content': 'This is the content'};
var myNoti = new Manager.Notification('TYPE', dictionary);
myNoti.showDictionary();
Manager.post(myNoti);
*/
 ...
 *
 * var Manager = require('tizen-notification');
 * var myNoti = new Manager.Notification('TYPE_NOTI', dictionary);
 * myNoti.content = "New contents";
 * myNoti.setEventHandler(CLICKBUTTON1, appControl);
 * Manager.post(myNoti);
 *
 * Manager.remove(myNoti);
 * Manager.removeAll();
 * ```
 */
