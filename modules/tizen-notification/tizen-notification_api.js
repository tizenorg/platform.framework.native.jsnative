...
class Manager {
  ...
  statusMessagePost(message) {
    if (!message) {
      throw Error('Invalid parameter, message is missing');
    }
    return new Promise(function(resolve, reject) {
      var args = {'message': message};
      native_async_call('statusMessagePost', args, function(result) {
        if (result['result'] == 'OK') {
          console.log('statusMessagePost: OK');
          resolve();
        } else {
          console.log('statusMessagePost: Fail');
          reject(new Error(result['reason']));
        }
      });
    });
...
class Notification {
  ...
  setLaunchOption(type, option) {
    if (!type || (type != "TYPE_APPCONTROL")) {
      throw Error("Invalide type or missing type");
    }
    // TODO: implement
  }
 ...
 * myNoti2.progress = 0.1;
 * Manager.post(myNoti2);
 *
 * myNoti2.progress = 0.5;
 ...
 * Manager.statusMessagePost(message);
 *
 * ```
 */
