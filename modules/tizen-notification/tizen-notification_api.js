 ...
 * class descriptions.
 *
 * Here is a sample code for using the Notification module.
 * ```
...
*
* @method post
* @param {Notification.Notification|Notification.OnGoingNotification
...
*/
  post(notification) {
    ...
    if (result['result'] === 'OK') {
      ...
      notification.insertionTime_ = new Date(result['insertionTime'])
                                    || new Date();
      console.log('post: OK');
    }
  }
...
*
* @method update
* @param {Notification.OnGoingNotification} notification The on-going type
...
* @return {Notification.Notification|Notification.OnGoingNotification
* |Notification.ActiveNotification}
*  The notification
* @since 3.0
*/
...
* * 'NONE' - Default
* * 'EVENT_SINGLE' - Used to inform single event
* * 'EVENT_MULTIPLE' - Used to inform multiple event
* * 'THUMBNAIL' - Used to display images
* * 'ONGOING_EVENT' - Used to display text message
* * 'ONGOING_PROGRESS' - Used to display progress
* @since 3.0
*/
class Notification {
...
* @param {Number} [customColor] The custom led color in numerical RGB value
...
*```
* ledBlinkPeriod: { ledOnPeriod: 100, ledOffPeriod: 100 }
*```
* @since 3.0
*/
...
* * 'operation' - 'LED_ON' or 'LED_OFF' in String type.
* * 'customColor' - The custom RGB led color in Number type. This is optional.
* * 'ledBlinkPeriod' - The time period of flashing the LED in ms (ledOnPeriod,
* ledOffPeriod)  This is optional.
*```
* led = { operation: 'LED_ON',
*         ledBlinkPeriod: { ledOnPeriod: 100, ledOffPeriod: 100 } }
*```
* @since 3.0
*/
...
* * 'TYPE_NOTI' - A basic notification type that is removed automatically when
 * selected by the user. All TY_NOTI type notifications can be removed by user
 * interaction.
* * 'TYPE_ONGOING' - A notification that informs the user whether an application
...
* the notification.
* * 'TYPE_ACTIVE' - a notification that is showed on the upper side of the
