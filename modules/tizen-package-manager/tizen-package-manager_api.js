 ...
 */
class Privilege {
  constructor(privilege_name) {
    console.log("@@@ Privilege constructor()");
...
class Package {
  constructor(pkg_id, pkg_label, pkg_iconpath, pkg_version, pkg_type,
              pkg_installedStorage, pkg_rootpath, pkg_systempackage,
              pkg_removable, pkg_preloaded, pkg_accessible) {
   ...
   * @async
   * @return {Promise|PackageManager.AppId}
   * @throws
   * * TypeError
   * * NotFoundError
   **/
  appsId(componentType) {
    console.log("@@@ apps()");
	var packageId = this[PACKAGE_ID];
    return new Promise(function(resolve, reject) {
      var args = {'componentType': componentType, 'packageId': packageId};
      native_async_call('appsId', args, function(result) {
        if (result['reason']) {
          reject(new Error(result['reason']));
          console.log("$$$ Error");
        }
        var apps = result['apps'].map(function(ids) {
          return new AppId(ids['appId']);
        });
        resolve(apps);
      });
    });
   ...
   * @method privileges
   * @async
   * @return {Promise|PackageManager.Privilege}
   * @throws
   * * TypeError
   **/
  privileges() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      native_async_call('privileges', args, function(result) {
        if (result['reason']) {
          reject(new Error(result['reason']));
          console.log("$$$ Error");
          return;
        }
        var privilege = result['privileges'].map(function(name) {
          return new Privilege(name['name']);
        });
        resolve(privilege);
      });
    });
...
class PackageManager extends EventEmitter {
  ...
  getPackage(packageId) {
    ...
    if (result['result'] == 'OK') {
      ...
      return new Package(pkg['id'], pkg['label'], pkg['iconPath'], pkg['version'], pkg['type'],
                         pkg['installedStorage'], pkg['rootPath'], pkg['systemPackage'],
                         pkg['removable'], pkg['preloaded'], pkg['accessible']);
    } else {
      console.log("$$$ Error");
      return undefined;
    }
  ...
  getPackages() {
    ...
    return new Promise(function(resolve, reject) {
      //var args = {'filter': filter};
      //args['filter'] = filter;
      //native_async_call('getPackages', args, function(result) {
      native_async_call('getPackages', undefined, function(result) {
        if (result['reason']) {
          reject(new Error(result['reason']));
          console.log("$$$ Error");
          return;
        ...
        var packages = result['packages'].map(function(pkgs) {
          return new Package(pkgs['id'], pkgs['label'], pkgs['iconPath'], pkgs['version'], pkgs['type'],
                             pkgs['installedStorage'], pkgs['rootPath'], pkgs['systemPackage'],
                             pkgs['removable'], pkgs['preloaded'], pkgs['accessible']);
        });
        resolve(packages);
      });
    });
...
.then(function(packageSize) {
  console.log("3. getPackageSize() "+packageSize);
  console.log("3-1. data : "+packageSize.data);
  console.log("3-2. cache : "+packageSize.cache);
  console.log("3-3. app : "+packageSize.app);
  console.log("3-4. externalData : "+packageSize.externalData);
  console.log("3-5. externalCache : "+packageSize.externalCache);
  console.log("3-6. externalApp : "+packageSize.externalApp);  
}).catch(function(e){
  console.log(e.message);
});

PackageManager.getPackage('pkgname').appsId('all-app')
.then(function(apps) {
  for (var cnt in apps) {
    console.log("4-1. all-app : "+apps[cnt].id);
}
}).catch(function(e){
  console.log(e.message);
});
PackageManager.getPackage('pkgname').appsId('ui-app')
.then(function(apps) {
  for (var cnt in apps) {
    console.log("4-2. ui-app : "+apps[cnt].id);
  }
}).catch(function(e){
  console.log(e.message);
});
PackageManager.getPackage('pkgname').appsId('service-app')
.then(function(apps) {
  for (var cnt in apps) {
    console.log("4-3. service-app : "+apps[cnt].id);
  }
}).catch(function(e){
  console.log(e.message);
});

PackageManager.getPackage('pkgname').privileges()
.then(function(privilege) {
  console.log("5. privileges() "+privilege);
  for (var cnt in privilege) {
    console.log("5-1. name "+privilege[cnt].name);
  }
}).catch(function(e){
  console.log(e.message);
});
