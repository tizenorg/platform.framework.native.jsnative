    ...
    return function(json) {
      ...
      if (msg['asyncId'] && async_map.has(msg['asyncId'])) {
        ...
        if (cb instanceof Function) {
          ...
          console.err("cb is not function");
        }
      } else {
 ...
 */
class PackageSize {
  constructor(sizeData, sizeCache, sizeApp, sizeExtData, sizeExtCache, sizeExtApp) {
...
var availableKeys = ['removable', 'readonly', 'supportDisable', 'disable', 'preload'];
var convertKey = {'removable': 'PACKAGE_MANAGER_PKGINFO_PROP_REMOVABLE',
  ...
  getPackageSize(packageId) {
    return new Promise(function(resolve, reject) {
      ...
      native_async_call('getPackageSize', args, function(result) {
        ...
        var packageSize = result['packageSize'];
        resolve(new PackageSize(packageSize['data'], packageSize['cache'], packageSize['app'],
                                packageSize['externalData'], packageSize['externalCache'], packageSize['externalApp']));
      });
    });
  }
};
