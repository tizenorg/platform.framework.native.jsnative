  ...
  getPackagesInfo() {
    ...
    return new Promise(function(resolve, reject) {
      native_async_call('getPackagesInfo', undefined, function(result) {
		...
		var packagesInfo_ = new PackageInfo();
        var packagesInfo = result['packagesInfo'].map(function(packagesInfo) {
          for (let info in packagesInfo) {
            if (info == 'id') {packagesInfo_.id_ = packagesInfo[info];}
            if (info == 'name') {packagesInfo_.name_ = packagesInfo[info];}
            if (info == 'iconPath') {packagesInfo_.iconPath_ = packagesInfo[info];}
            if (info == 'version') {packagesInfo_.version_ = packagesInfo[info];}
            if (info == 'type') {packagesInfo_.type_ = packagesInfo[info];}
            if (info == 'installedStorage') {packagesInfo_.installedStorage_ = packagesInfo[info];}
            if (info == 'rootPath') {packagesInfo_.rootPath_ = packagesInfo[info];}
            if (info == 'isSystemPackage') {packagesInfo_.isSystemPackage_ = packagesInfo[info];}
            if (info == 'isRemovablePackage') {packagesInfo_.isRemovablePackage_ = packagesInfo[info];}
            if (info == 'isPreloadPackage') {packagesInfo_.isPreloadPackage_ = packagesInfo[info];}
            if (info == 'isAccessible') {packagesInfo_.isAccessible_ = packagesInfo[info];}
          }
        return packagesInfo_;
        });
  ...
  getPackageSizeInfo(packageId) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      native_async_call('getPackageSizeInfo', args, function(result) {
        ...
        var packageSizeInfo = result['packageSizeInfo'].map(function() {
		  ...
		  return packageSizeInfo_;
        });
        resolve(packageSizeInfo);
      });
    });
  }
