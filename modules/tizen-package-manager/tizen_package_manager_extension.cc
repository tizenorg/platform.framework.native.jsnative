...
void PackageManagerInstance::HandleGetPackages(const std::string& asyncId, const picojson::value& filter) {
  ...
  LOGD("!!!!!!!!!!!!!!!! filter %s", filter.serialize().c_str());


  //picojson::value value = picojson::value(filter);
  //std::string str = picojson::value(value).serialize();
  ...
  if (!filter.is<picojson::null>()) {
    ...
    for (auto& item : keys) {
      auto key = item.first;
      LOGD("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %s", key.c_str());
      LOGD("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d", keys[key].get<bool>());
      bool value = keys[key].get<bool>();
  ...
  std::string str1 = picojson::value(result).serialize();
  LOGD("??????????????????????????????? 1 %s", str1.c_str());
