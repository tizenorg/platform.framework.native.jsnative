...
/*
  ...
  if (ret == 0) {
    result["informationArray"] = picojson::value(array);
  } else {
...
/*
  char* tepName = NULL;
  ret = pkgmgrinfo_pkginfo_get_tep_name(info, &tepName);
  if ((ret != PMINFO_R_OK) || (tepName == NULL)) {
    LOGE("[%s] Failed to get package tepName", id);
    return false;
  }
  result["tepName"] = picojson::value(tepName);
*/
...
void PackageManagerInstance::HandleGetPackageInfo(const std::string& pkgId) {
  ...
  //int ret = package_manager_info_check_privilege();
  ...
  if (pkgmgrinfo_pkginfo_get_usr_pkginfo(pkgId.c_str(), getuid(), &info) != PMINFO_R_OK) {
    LOGE("Failed to get pkginfo");
    return;
  }

  picojson::object object;
  if (!ConvertToPackageToObject(info, object)) {
