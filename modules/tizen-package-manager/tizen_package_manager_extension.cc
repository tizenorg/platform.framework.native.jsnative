...
// async
void PackageManagerInstance::HandleMessage(const char* msg) {
...
static bool package_info_app_cb(package_info_app_component_type_e comp_type,
                                    const char *appId, void *user_data) {
  LOGD("Enter package_info_app_cb()");

  picojson::array* arrayData = static_cast<picojson::array*>(user_data);
  if (!arrayData) {
...
static bool package_info_privilege_info_cb(
                                  const char *privilegeName, void *user_data) {
...
bool PackageManagerInstance::ConvertToPackageToObject(
  ...
  ret = package_info_foreach_app_from_package(
                 info, PACKAGE_INFO_ALLAPP, package_info_app_cb, &arrayAppIds);
  if (ret != PACKAGE_MANAGER_ERROR_NONE) {
  ...
  ret = package_info_foreach_privilege_info(
                        info, package_info_privilege_info_cb, &arrayPrivilege);
  if (ret != PACKAGE_MANAGER_ERROR_NONE) {
...
void PackageManagerInstance::HandleGetPackage(
...
static bool package_manager_package_info_cb(
                                  const package_info_h info, void* user_data) {
...
void PackageManagerInstance::HandleGetPackages(
  ...
  if (filter_h != nullptr) {
    int ret = package_manager_filter_foreach_package_info(
                            filter_h, package_manager_package_info_cb, &array);
    if (ret != PACKAGE_MANAGER_ERROR_NONE) {
    ...
    int ret = package_manager_foreach_package_info(
                                      package_manager_package_info_cb, &array);
    if (ret != PACKAGE_MANAGER_ERROR_NONE) {
...
static void package_manager_size_info_receive_cb(
  const char* packageId, const package_size_info_h sizeInfo, void* user_data) {
  ...
  object["data"] =
                  picojson::value(std::to_string(sizeInfo->data_size).c_str());
  object["cache"] =
                 picojson::value(std::to_string(sizeInfo->cache_size).c_str());
  object["app"] =
                   picojson::value(std::to_string(sizeInfo->app_size).c_str());
  object["externalData"] =
         picojson::value(std::to_string(sizeInfo->external_data_size).c_str());
  object["externalCache"] =
        picojson::value(std::to_string(sizeInfo->external_cache_size).c_str());
  object["externalApp"] =
          picojson::value(std::to_string(sizeInfo->external_app_size).c_str());
...
void PackageManagerInstance::HandleGetPackageSize(
                        const std::string& asyncId, const std::string& pkgId) {
  ...
  int ret = package_manager_get_package_size_info(
                    pkgId.c_str(), package_manager_size_info_receive_cb, data);
  if (ret != PACKAGE_MANAGER_ERROR_NONE) {
...
EXPORT_XWALK_EXTENSION(
               tizen_package_manager, packagemanager::PackageManagerExtension);
