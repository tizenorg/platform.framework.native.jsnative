...
void PackageManagerInstance::HandleMessage(const char* msg) { //async
  ...
  if (cmd == "getPackages") {
    ...
    HandleGetPackageSize2(asyncId, pkgId);
  } else {
...
void PackageManagerInstance::HandleGetPackageSize2(const std::string& asyncId, const std::string& pkgId) {
