...
class SoundManagerInstance : public xwalk::XWalkExtensionInstance {
 public:
  ...
  static void InterruptListener(sound_session_interrupted_code_e type,
                                void *userData);
 ...
 private:
  ...
  const picojson::object SetSessionInterruptOption(
      const picojson::value& value);
  const picojson::object GetSessionResumptionOption();
  const picojson::object SetSessionResumptionOption(
      const picojson::value& value);
