...
class SoundManager extends EE{
  constructor() {
    super();
    this[connection_filter] = {
        'direction': 'both', 'type': 'all', 'state': 'all'
    };
    this[deviceinfo_filter] = {
        'direction': 'both', 'type': 'all', 'state': 'all'
    };
  ...
  __event_handle__(msg) {
    if (msg['event'] === 'connectionState') {
      this.emit('connectionState', msg['data']);
    } else if (msg['event'] === 'deviceInfo') {
      this.emit('deviceInfo', msg['data']);
    } else {
   ...
   *                  ', io-direction:' + device.ioDirection +
   *                  ', type:' + device.type + ', state:' + device.state);
   *      }
   *    }).catch(function(e) {
...
class Volume extends EE{
  ...
  set currentSoundType(type) {
    ...
    native_sync_call('setcurrentSoundType', args);
  }
  ...
  setVolume(type, volume) {
    ...
    native_sync_call('setVolume', args);
  }
...
class Session extends EE {
  ...
  constructor() {
    ...
    if (result['result'] === 'FAIL') {
      console.error('error mesage: ' + result['reason']);
    }
    registerEventHandler(this);
  }
  ...
  __event_handle__(event) {
    if (event['event'] === 'sessionInterrupt') {
      ...
      console.error('invalid event type was passed');
    }
  }
  ...
  set type(type) {
    if (!SessionType[type]) {
      console.error('invalid value is passed');
      return;
    }
  ...
  set startingOption(option) {
    ...
    var result = native_sync_call('setSessionStartingOption',
                                  {'option': option});
    if (result['result'] === 'FAIL') {
      console.error('error mesage: ' + result['reason']);
    }
  ...
  set interruptOption(option) {
    ...
    var result = native_sync_call('setSessionInterruptOption',
                                  {'option': option});
    if (result['result'] === 'FAIL') {
      console.error('error mesage: ' + result['reason']);
    }
  ...
  set resumptionOption(option) {
    ...
    var result = native_sync_call('setSessionResumptionOption',
                                  {'option': option});
    if (result['result'] === 'FAIL') {
      console.error('error mesage: ' + result['reason']);
    }
