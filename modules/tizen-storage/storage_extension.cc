...
static void CallbackOnChange(int storage_id, storage_state_e state, void* user_data) {
  StorageInstance* instance = static_cast<StorageInstance*>(user_data);
  ...
  instance->FireSimpleEvent("change", change);
}
...
//  ret = storage_set_state_changed_cb(storage_id, CallbackOnChange, user_data);
//  ret = storage_set_state_changed_cb(storage_id, CallbackOnChange, static_cast<void*>(this));
...
void StorageInstance::HandleGetStorages(const std::string& asyncid) {
  ...
  int ret = 0;
  ret = storage_foreach_device_supported(CallbackGetStorages, &array);
