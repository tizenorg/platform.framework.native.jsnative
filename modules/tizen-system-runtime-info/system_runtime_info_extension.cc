...
void SystemRuntimeInfoInstance::GetProcessMemoryInfo(picojson::array& pidList) {
  ...
  if (ret != RUNTIME_INFO_ERROR_NONE){
    ...
    for(int i=0; i<size; i++){
		picojson::value::object obj;
		obj["vsz"] = picojson::value(std::to_string(memoryList[i].vsz));
		obj["rss"] = picojson::value(std::to_string(memoryList[i].rss));
		obj["pss"] = picojson::value(std::to_string(memoryList[i].pss));
		obj["shared_clean"] = picojson::value(std::to_string(memoryList[i].shared_clean));
		obj["shared_dirty"] = picojson::value(std::to_string(memoryList[i].shared_dirty));
		obj["private_clean"] = picojson::value(std::to_string(memoryList[i].private_clean));
		obj["private_dirty"] = picojson::value(std::to_string(memoryList[i].private_dirty));

		LOGD("[%d , %d, %d]", memoryList[i].vsz, memoryList[i].rss, memoryList[i].pss);

		retArray.push_back(picojson::value(obj));
	}
    result["result"] = picojson::value("SUCCESS");
    result["memoryInfos"] = picojson::value(retArray);
