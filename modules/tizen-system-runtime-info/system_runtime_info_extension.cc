...
void SystemRuntimeInfoInstance::GetValue(const std::string& key) {
	...
	auto iterInt = runtimeInfoKeyIntMap.find(key);
	if(iterInt != runtimeInfoKeyIntMap.end()){
		LOGD("finded");
		GetValueInt(key);
		return;
	}

	auto iterBool = runtimeInfoKeyBoolMap.find(key);
	if(iterBool != runtimeInfoKeyBoolMap.end()){
		LOGD("finded");
		GetValueInt(key);
		return;
	}
...
void SystemRuntimeInfoInstance::GetValueInt(const std::string& key) {
  ...
  int ret = runtime_info_get_value_bool(RUNTIME_INFO_KEY_BLUETOOTH_ENABLED, &value);
