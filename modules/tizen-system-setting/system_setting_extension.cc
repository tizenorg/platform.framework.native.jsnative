...
SystemSettingInstance::SystemSettingInstance() {
  ...
  for (auto iter = settingKeys.begin(); iter !=  settingKeys.end(); iter++) {
    ...
    if (ret != SYSTEM_SETTINGS_ERROR_NONE) {
      ...
      LOGE("Is is failed to register listener: %s (%s)", keyStr.c_str(), (getErrorMessage(ret)).c_str());
    }
  }
}
...
SystemSettingInstance::~SystemSettingInstance() {
  ...
  for (auto iter = settingKeys.begin(); iter !=  settingKeys.end(); iter++) {
    ...
    if (ret != SYSTEM_SETTINGS_ERROR_NONE) {
      std::string keyStr = iter->first;
      LOGE("Is is failed to remove listener: %s (%s)", keyStr.c_str(), (getErrorMessage(ret)).c_str());
    }
  }
}
...
void SystemSettingInstance::setValue(const picojson::value& args, picojson::object& out) {
  ...
  LOGD("[DEBUG_NATIVE] SET key: %s(%s), value: %s", key.c_str(), type.c_str(), value.c_str());
  ...
  if (ret == SYSTEM_SETTINGS_ERROR_NONE) {
    ...
    LOGE("Is is failed to set system setting value: %s - %s", key.c_str(), (getErrorMessage(ret)).c_str());
...
void SystemSettingInstance::getValue(const picojson::value& args, picojson::object& out) {
  ...
  LOGE("Is is failed to get system setting value: %s - %s", key.c_str(), (getErrorMessage(ret)).c_str());
