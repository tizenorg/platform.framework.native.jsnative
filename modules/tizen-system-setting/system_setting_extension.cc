...
static const std::unordered_map<std::string,
  system_settings_key_e> settingKeys = {
   {
    ...
    {
      "sound-notification-repetition-period",
...
static const std::unordered_map<std::string,
  system_settings_font_size_e> fontSizeValues = {
    {
      "small",
...
static const std::unordered_map<std::string,
  system_settings_idle_lock_state_e> lockStateValues = {
    {
      "unlock",
...
SystemSettingInstance::~SystemSettingInstance() {
  ...
  for (auto it = settingKeys.begin(); it != settingKeys.end(); ++it) {
    ...
    if (ret != SYSTEM_SETTINGS_ERROR_NONE) {
      std::string keyStr = it->first;
      LOGE("It is failed to remove listener: %s (%s)", keyStr.c_str(),
           (getErrorMessage(ret)).c_str());
    }
  }
}
...
void SystemSettingInstance::setValue(const picojson::value& args,
  ...
  LOGD("[DEBUG_NATIVE] SET key: %s(%s), value: %s",
...
void SystemSettingInstance::getValue(const picojson::value& args,
  ...
  if (type == TYPE_INT) {
    ...
    if (ret == SYSTEM_SETTINGS_ERROR_NONE) {
      ReportSuccess(picojson::value(static_cast<double>(val)), out);
      return;
    }
  } else if (type == TYPE_STRING) {
    char* val;
    ...
    if (ret == SYSTEM_SETTINGS_ERROR_NONE) {
      std::string retVal = std::string(val);
    ...
    if (ret == SYSTEM_SETTINGS_ERROR_NONE) {
      ReportSuccess(picojson::value(val), out);
      return;
    }
  } else {
    LOGE("Invalid Type");
...
std::string SystemSettingInstance::getErrorMessage(const int err) {
  switch (err) {
